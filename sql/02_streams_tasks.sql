-- Streams and Tasks pipeline (after-stream chaining)
USE ROLE ACCOUNTADMIN;
USE DATABASE MLP_DEMO_DB;
USE SCHEMA MLP_DEMO_DB.RAW;

-- Streams on raw tables
CREATE OR REPLACE STREAM ORDERS_RAW_STREAM ON TABLE ORDERS_RAW;
CREATE OR REPLACE STREAM EVENTS_RAW_STREAM ON TABLE EVENTS_RAW;
CREATE OR REPLACE STREAM INVENTORY_RAW_STREAM ON TABLE INVENTORY_RAW;

-- Stage schema objects
USE SCHEMA MLP_DEMO_DB.STAGE;
CREATE OR REPLACE TABLE ORDERS_ENRICHED LIKE MLP_DEMO_DB.RAW.ORDERS_RAW;
CREATE OR REPLACE TABLE EVENTS_FLATTENED (
  EVENT_TS TIMESTAMP_NTZ,
  USER_ID STRING,
  EVENT_TYPE STRING,
  EVENT_ATTR VARIANT
);

-- Snowpark proc will write into STAGE.*; tasks will call it
-- Task chain: task_transform_stage (AFTER stream) -> task_publish_curated (AFTER previous)
CREATE OR REPLACE TASK task_transform_stage
  WAREHOUSE = MLP_TRANSFORM_WH
  AFTER ORDERS_RAW_STREAM
AS
  CALL MLP_DEMO_DB.PUBLIC.SP_TRANSFORM_ORDERS();

-- Publish/curate step (simple example insert-select)
USE SCHEMA MLP_DEMO_DB.CURATED;
CREATE OR REPLACE TABLE ORDER_METRICS AS SELECT * FROM MLP_DEMO_DB.STAGE.ORDERS_ENRICHED WHERE 1=0;

CREATE OR REPLACE TASK task_publish_curated
  WAREHOUSE = MLP_TRANSFORM_WH
  AFTER task_transform_stage
AS
  INSERT INTO ORDER_METRICS
  SELECT * FROM MLP_DEMO_DB.STAGE.ORDERS_ENRICHED;

-- Enable tasks
ALTER TASK MLP_DEMO_DB.STAGE.task_transform_stage RESUME;
ALTER TASK MLP_DEMO_DB.CURATED.task_publish_curated RESUME;

